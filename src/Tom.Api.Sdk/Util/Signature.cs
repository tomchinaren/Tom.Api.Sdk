    ...
    public class Signature
    {
        ...
        public static string RSASignCharSet(string data, string privateKeyPem, string charset, bool keyFromFile, string signType)
        {
            ...
            try
            {
                ...
                if (null == rsaCsp)
                {
                    throw new ApiException("您使用的私钥格式错误，请检查RSA私钥配置" + ",charset = " + charset);
                }
                if ("RSA2".Equals(signType))
                {
        ...
        public static string CheckSignAndDecrypt(IDictionary<string, string> parameters, string publicKey,
                                             ...
                                             bool isDecrypt)
        {
            ...
            if (isCheckSign)
            {
                if (!RSACheckV2(parameters, publicKey, charset))
                {
                    throw new ApiException("rsaCheck failure:rsaParams=" + parameters);
                }
            }
                                             ...
                                             bool isDecrypt, string signType, bool keyFromFile)
        {
            ...
            if (isCheckSign)
            {
                if (!RSACheckV2(parameters, publicKey, charset, signType, keyFromFile))
                {
                    throw new ApiException("rsaCheck failure:rsaParams=" + parameters);
                }
            }
        ...
        public static string encryptAndSign(string bizContent, string publicKey,
                                        ...
                                        bool isSign, string signType, bool keyFromFile)
        {
            ...
            {// 加密
                sb.Append("<security>");
                String encrypted = RSAEncrypt(bizContent, publicKey, charset, keyFromFile);
        ...
        public static string RSADecrypt(string content, string privateKeyPem, string charset, string signType)
        {
            ...
            {
                throw new ApiException("DecryptContent = " + content + ",charset = " + charset, ex);
            }
        }
        ...
        private static RSACryptoServiceProvider LoadCertificateString(string strKey, string signType)
        {
            ...
            {
                //    throw new ApiException("EncryptContent = woshihaoren,zheshiyigeceshi,wanerde", ex);
            }
            return null;
        }
